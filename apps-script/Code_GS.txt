    ...
    case "toggleClearedMultiple": {
      ...
      for (let i = 1; i < sheetData.length; i++) {
        ...
        if (data.oldStates[id] !== undefined) {
          sheet.getRange(i + 1, clearedIndex + 1).setValue(!data.oldStates[id]);
        }
      }
      break;
    }
...
// ================= BULK DUPLICATE =================
function duplicateMultiple(ids) {
  const sheet = getSheet(SHEETS.TRANSACTIONS);
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  const idIndex       = headers.indexOf("id");
...
function reconcileTransactions(ids) {
  ...
  // Verrouiller toutes les transactions de la liste
  for (let i = 1; i < data.length; i++) {
    if (idSet.has(String(data[i][idIndex]))) {
      sheet.getRange(i + 1, lockedIndex + 1).setValue(true);
    }
